...
| MiniMax-H3 | `minimax-h3` | APIMart / Pika | text-to-video, first-frame, first-last-frame, image-ref, video-ref | `duration` (4–15s), `resolution` (2K/768P), `aspect_ratio` (Adaptive + 6 fixed ratios), `watermark` |
...
Pika MiniMax-H3 uses the same Bragi model ID and five modes, routed to Pika's text, image, and reference endpoints. Pika's reference route also supports audio-only input: select `video-ref` when there is only audio, and cite the inputs as `@Audio1`, `@Image1`, or `@Video1`. It accepts at most 9 images, 3 videos, 3 audio clips, and 12 files total; video references total at most 15 seconds. Bragi uploads local files through Relay before submission. Pika's text route maps Adaptive ratio to 16:9; frame modes omit ratio. Pika maps `watermark` to `aigc_watermark`.

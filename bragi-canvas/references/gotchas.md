...
- Async types (video and providers such as Mureka or Sonilo Music): use `list_pending_tasks` and `get_task_status(taskId)` after submission. When the task disappears from the queue, check the placeholder. These tasks persist across plugin restarts and reopen with the matching canvas. Sonilo `video-to-music` requires exactly one upstream video and sends it through Bragi Relay; `music` uses text alone.
...
Retry countdowns and paused tasks stop the generating animation. Paused tasks show a reason and a circular arrow button beside the bottom model pill, with the tooltip **Resume checking**; the placeholder menu and command palette also offer resume. Credentials, unknown responses and local storage errors pause immediately. Resume opens a fresh 15-minute window for the same task ID and never resubmits generation. Do not call `generate` to recover it. `status: "pending"` in MCP means retained, not necessarily actively checking; inspect `state` and `lastError`.

SVRouter video placeholders show input preparation, reference upload/registration, waiting for reference approval, video submission, provider waiting, download and save stages. A reference-registration HTTP 502 means video submission has not started. Reference reads and gateway requests time out after 60 seconds, relay uploads after three minutes, and reference approval has a five-minute polling budget (plus an in-flight request). Video submission times out after 90 seconds without automatically replaying the POST; acceptance is uncertain in that case, so check with the provider before submitting again. Shared status requests time out after two minutes; SVRouter video downloads after three minutes. A timeout cannot cancel Obsidian's underlying HTTP operation. Pre-submission reference failures have no queued video task to resume.
